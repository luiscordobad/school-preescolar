...
export default async function DashboardPage() {
  ...
  const typedClassrooms = (classrooms ?? []) as Database["public"]["Tables"]["classroom"]["Row"][];
  ...
  const typedStudents = (students ?? []) as Database["public"]["Tables"]["student"]["Row"][];
    ...
    <main className="flex flex-1 flex-col gap-6">
      <section className="rounded-lg border border-slate-200 bg-white p-6 shadow-sm">
        ...
        <p className="mt-2 text-sm text-slate-500">
          Bienvenido{typedProfile?.display_name ? `, ${typedProfile.display_name}` : ""}. Tu rol es <strong>{typedProfile?.role}</strong>
      ...
      <section className="grid gap-6 md:grid-cols-2">
        <article className="rounded-lg border border-slate-200 bg-white p-6 shadow-sm">
          ...
          <ul className="mt-4 space-y-2">
            {typedClassrooms.length ? (
              typedClassrooms.map((classroom) => (
